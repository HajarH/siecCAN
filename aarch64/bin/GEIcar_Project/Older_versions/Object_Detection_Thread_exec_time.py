...
class Detection(Thread):
    ...
    def run(self):
            ...
            while (loop_counter < NUMBER_OF_ITERATION):
                delta_time_loop = time.clock() #Measure execution time START
                ...
                delta_time_loop = time.clock() - delta_time_loop #Measure execution time END
                #Store the value for later analysis
                glob.tab_delta_time_loop = np.append(glob.tab_delta_time_loop, delta_time_loop)
